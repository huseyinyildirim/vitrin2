...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_iletisimler
							select new
							{
								p.id,
								p.ad_soyad,
	...
	#region Okuma İşlemleri

	[DirectMethod]
	public void OkumaEkraniGetir(int ID)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_iletisimler
						where p.id == ID
						select new
						{
							p.telefon,
			...
			if (SQL2.Count() > 0)
			{
				...
				Label_Okuyan.Text = SQL2.Select(x => x.guncelleyen).FirstOrDefault();
	...
	protected void SecilenleriOku(object sender, EventArgs e)
	{
		Oku(-1);
	}
