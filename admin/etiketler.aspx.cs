...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected bool EtiketKontrol(string Etiket, int DilID)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_etiketler
					   where p.ad == Etiket && p.dil_id == DilID
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				var SQL1 = (from p in SME.tbl_etiketler 
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.admin_id_gun,
					p.dil_id,
					ziyaret = SME.tbl_ziyaretler.Where(k => k.etiket_id == p.id).Count(),
	...
	{
		if (!EtiketKontrol(Class.Fonksiyonlar.Genel.KarakterTemizle(Class.Fonksiyonlar.Genel.TurkceKarakter(Ekle_TextFieldEtiket.Text.Trim()),false), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxDil.Value.ToString().Trim()))))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				ta.ad = Class.Fonksiyonlar.Genel.KarakterTemizle(Class.Fonksiyonlar.Genel.TurkceKarakter(Ekle_TextFieldEtiket.Text.Trim()),false);
		...
		{
			HataMesaji("Aynı etiketten mevcut.<br/>Lütfen farklı bir etiket belirtin!");
		}
	} 
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			if (SQL2.Count() > 0)
			{
				...
				Duzenle_Etiket.Text = SQL2.Select(x => x.ad).FirstOrDefault().ToString();
				Duzenle_ComboBoxDil.SelectedItem.Value = SQL2.Select(x => x.dil_id).FirstOrDefault().ToString();
				Duzenle_ComboBoxAnasayfa.SelectedItem.Value = Class.Fonksiyonlar.Genel.Cevirmeler.BoolToInteger(Class.Fonksiyonlar.Genel.Cevirmeler.StringToBool(SQL2.Select(x => x.anasayfa).FirstOrDefault().ToString()));
				Duzenle_TextFieldEtiket.Text = SQL2.Select(x => x.ad).FirstOrDefault();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_etiketler ta = (from p in SME.tbl_etiketler where p.id == PID select p).FirstOrDefault();
			ta.dil_id = int.Parse(Duzenle_ComboBoxDil.SelectedItem.Value.ToString());
			ta.ad = Class.Fonksiyonlar.Genel.KarakterTemizle(Class.Fonksiyonlar.Genel.TurkceKarakter(Duzenle_TextFieldEtiket.Text.Trim()),false);
			ta.anasayfa = Class.Fonksiyonlar.Genel.Cevirmeler.StringToBool(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxAnasayfa.Value.ToString().Trim()));
			ta.onay = OnayBool;
			ta.admin_id_gun = Class.Fonksiyonlar.Admin.Genel.AdminID();

			if (Duzenle_Etiket.Text != Class.Fonksiyonlar.Genel.KarakterTemizle(Class.Fonksiyonlar.Genel.TurkceKarakter(Duzenle_TextFieldEtiket.Text.Trim()),false))
			{
				if (!EtiketKontrol(Class.Fonksiyonlar.Genel.KarakterTemizle(Class.Fonksiyonlar.Genel.TurkceKarakter(Duzenle_TextFieldEtiket.Text.Trim()),false), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxDil.Value.ToString().Trim()))))
				{
					SME.SaveChanges();
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_etiketler ta = (from p in SME.tbl_etiketler where p.id == ID select p).FirstOrDefault();
