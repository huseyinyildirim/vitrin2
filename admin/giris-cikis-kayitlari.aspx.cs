	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_giris_cikislar_hatalar
					   select new
					   {
						   p.id,
						   p.admin_id,
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.islem,
					#region Admin Adı
					admin = SME.tbl_adminler.Where(k => k.id == p.admin_id).Select(x => "(" + x.kullanici_adi + ") " + x.ad_soyad).FirstOrDefault(),
					#endregion
					#region İşlem Tipi
					...
					p.kullanici_adi,
					#region IP Adresi
					ip = SME.tbl_all_http.Where(k => k.kayit_id == p.id && k.tablo == "tbl_giris_cikislar").Select(x => x.REMOTE_ADDR).FirstOrDefault(), 
					#endregion
					p.tarih
