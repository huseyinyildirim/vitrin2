<%@ Page Language="C#" AutoEventWireup="true" CodeFile="etiketler.aspx.cs" Inherits="yonetim_Kullanici"
...
<body>
	<script type="text/javascript">
		...
		var DuzenleKontrol = function () {
			...
			else if (!Duzenle_TextFieldEtiket.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Duzenle_ComboBoxAnasayfa.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir anasayfa durumunu seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
		...
		var KaydetKontrol = function () {
			...
			else if (!Ekle_TextFieldEtiket.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Ekle_ComboBoxAnasayfa.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir anasayfa durumunu seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_etiketler&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Etiketler" Layout="FitLayout"
				Icon="TagOrange" Closable="false" Minimizable="false" MinHeight="480" MinWidth="640">
				<Items>
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString"
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="ad" Header="Etiket" />
								<ext:Column DataIndex="anasayfa" Header="Anasayfa" Align="Center" Width="60">
									<Renderer Fn="AnasayfaRenk" />
								</ext:Column>
								<ext:Column DataIndex="ziyaret" Header="Ziyaret" MenuDisabled="true" Sortable="false"
									Width="55" />
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:StringFilter DataIndex="etiket" />
						...
						<Buttons>
							<ext:Button runat="server" ID="Button5" Text="Anasayfada Yayınla" Icon="BookLink">
								<DirectEvents>
									<Click OnEvent="AnasayfaEkle">
										...
										<EventMask ShowMask="true" MinDelay="500" Msg="Yükleniyor..." />
														...
														<ext:Button runat="server" ID="Button6" Text="Anasayfadan Kaldır" Icon="BookDelete">
								<DirectEvents>
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Ekle_TextFieldEtiket" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Etiket" BlankText="Lütfen etiketi yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan"
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField ID="Duzenle_Etiket" Hidden="true" runat="server" />
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldEtiket" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Etiket" BlankText="Lütfen bir etiket yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan"
						EnableKeyEvents="true">
						<Listeners>
							<KeyUp Handler="this.setValue(this.getValue().toLowerCase());" />
						</Listeners>
