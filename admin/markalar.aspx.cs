...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected bool MarkaKontrol(string Marka)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_markalar
					   where p.ad == Marka
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.admin_id_ek,
					p.admin_id_gun,
					ziyaret = SME.tbl_ziyaretler.Where(k => k.marka_id == p.id).Count(),
	...
	{
		if (!MarkaKontrol(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldMarka.Text.Trim()).ToUpper()))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				tbl_markalar ta = new tbl_markalar();
				ta.ad = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldMarka.Text.Trim()).ToUpper();
				ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
				ta.onay = OnayBool;
				SME.AddTotbl_markalar(ta);
				SME.SaveChanges();
		...
		{
			HataMesaji("Aynı markadan mevcut.<br/>Lütfen farklı bir marka belirtin!");
		}
	} 
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_markalar
						where p.id == ID
						select new
						{
							p.id,
			...
			if (SQL2.Count() > 0)
			{
				Duzenle_Marka.Text = SQL2.Select(x => x.ad).FirstOrDefault();
				Duzenle_TextFieldMarka.Text = SQL2.Select(x => x.ad).FirstOrDefault();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_markalar ta = (from p in SME.tbl_markalar where p.id == PID select p).FirstOrDefault();
			ta.ad = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldMarka.Text.Trim()).ToUpper();
			ta.onay = OnayBool;
			ta.admin_id_gun = Class.Fonksiyonlar.Admin.Genel.AdminID();

			if (Duzenle_Marka.Text != Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldMarka.Text.Trim()).ToUpper())
			{
				if (!MarkaKontrol(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldMarka.Text.Trim()).ToUpper()))
				{
					SME.SaveChanges();
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_markalar ta = (from p in SME.tbl_markalar where p.id == ID select p).FirstOrDefault();
