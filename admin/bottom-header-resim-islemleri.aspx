<%@ Page Language="C#" AutoEventWireup="true" CodeFile="bottom-header-resim-islemleri.aspx.cs" Inherits="yonetim_Kullanici"
...
<body>
	<script type="text/javascript">
		...
		var OtherYol = function (data) {
			return String.format("<a href=\"" + data.replace(data, "../images/other/" + data) + "\" target=\"_blank\">" + data + "</a>");
		};
		...
		var WindowEkleClear = function () {
			for (var i = 0; i <= WindowEkle.items.items.length; i++) {
				if (WindowEkle.items.items[i].xtype == "textfield") {
					...
					if (WindowEkle.items.items[i].id == "Ekle_ComboBoxOnay" || WindowEkle.items.items[i].id == "Ekle_ComboBoxYer" || WindowEkle.items.items[i].id == "Ekle_ComboBoxKonum") {
		...
		var DuzenleKontrol = function () {
			...
			else if (Duzenle_ComboBoxResim.getSelectedIndex() == -1) {
		...
		var KaydetKontrol = function () {
			...
			else if (Ekle_ComboBoxResim.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir resim seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_ikonlar&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Bottom/Header Resim İşlemleri" Layout="FitLayout"
				Icon="PictureSave" Closable="false" Minimizable="false" MinHeight="480" MinWidth="640">
				<Items>
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString"
						AutoExpandColumn="ikon" OnBeforeClientInit="GridPanelYuklenmedenOnce" AnchorHorizontal="100%"
								...
								<Reader>
									<ext:JsonReader IDProperty="id">
										<Fields>
											...
											<ext:RecordField Name="alt" />
											<ext:RecordField Name="yer" />
											<ext:RecordField Name="yers" />
											<ext:RecordField Name="konum" />
											<ext:RecordField Name="konums" />
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="alt" Header="Açıklama" />
								<ext:Column DataIndex="ikon" Header="İkon">
									<Renderer Fn="OtherYol" />
								</ext:Column>
								<ext:Column DataIndex="yer" Header="Yer ID" Hidden="true" />
								<ext:Column DataIndex="yers" Header="Yer" MenuDisabled="true" Sortable="false" Width="30" />
								<ext:Column DataIndex="konum" Header="Konum ID" Hidden="true" />
								<ext:Column DataIndex="konums" Header="Konum" MenuDisabled="true" Sortable="false"
									Width="45" />
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:NumericFilter DataIndex="konum_id" />
									<ext:NumericFilter DataIndex="yer_id" />
									<ext:StringFilter DataIndex="baslik" />
									<ext:StringFilter DataIndex="resim" />
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:ComboBox ID="Ekle_ComboBoxYer" runat="server" FieldLabel="Yer" Editable="false"
						Selectable="true">
						<Items>
							<ext:ListItem Text="ÜST" Value="0" />
							<ext:ListItem Text="ALT" Value="1" />
						</Items>
						<SelectedItem Value="0" />
					</ext:ComboBox>
					<ext:ComboBox ID="Ekle_ComboBoxKonum" runat="server" FieldLabel="Konum" Editable="false"
						Selectable="true">
						<Items>
							<ext:ListItem Text="SOL" Value="0" />
							<ext:ListItem Text="SAĞ" Value="1" />
						</Items>
						<SelectedItem Value="0" />
					</ext:ComboBox>
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:ComboBox ID="Duzenle_ComboBoxResim" FieldLabel="Resim" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan"
						runat="server" DisplayField="resim" ValueField="resim" Editable="true" TypeAhead="true"
						Mode="Local" ForceSelection="true" SelectOnFocus="true" EmptyText="Resim Seçiniz...">
						<Store>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldBaslik" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Başlık" BlankText="Lütfen bir başlık yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
					<ext:ComboBox ID="Duzenle_ComboBoxYer" runat="server" FieldLabel="Yer" Editable="false"
						Selectable="true">
						<Items>
							<ext:ListItem Text="ÜST" Value="0" />
							<ext:ListItem Text="ALT" Value="1" />
					...
					<ext:ComboBox ID="Duzenle_ComboBoxKonum" runat="server" FieldLabel="Konum" Editable="false"
						Selectable="true">
						<Items>
							<ext:ListItem Text="SOL" Value="0" />
							<ext:ListItem Text="SAĞ" Value="1" />
