<%@ Page Language="C#" AutoEventWireup="true" CodeFile="diller.aspx.cs" Inherits="yonetim_Kullanici"
...
<body>
	...
	<script type="text/javascript">
		...
		var DugmelerinDurumu = function (grid, toolbar, rowIndex, record) {
			...
			switch (record.data.ana_dil) {
				case "EVET":
					toolbar.items.get(2).setDisabled(true);
					toolbar.items.get(2).setTooltip("Ana Dil üzerinde işlem yapamazsınız.");

					toolbar.items.get(4).setDisabled(true);
					toolbar.items.get(4).setTooltip("Ana Dil üzerinde işlem yapamazsınız.");

					toolbar.items.get(6).setDisabled(true);
					toolbar.items.get(6).setTooltip("Ana Dil üzerinde işlem yapamazsınız.");
					break;
			}
		};
		...
		var DuzenleKontrol = function () {
			if (!Duzenle_TextFieldDil.validate() || !Duzenle_TextFieldKodlama.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Duzenle_ComboBoxUlke.getSelectedIndex() == -1 || Duzenle_ComboBoxUlke.getValue() == "") {
		...
		var KaydetKontrol = function () {
			if (!Ekle_TextFieldDil.validate() || !Ekle_TextFieldKodlama.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Ekle_ComboBoxUlke.getSelectedIndex() == -1 || Ekle_ComboBoxUlke.getValue() == "") {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir ülke seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_diller&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
		...
		var IISRESET = function () {
			Ext.Msg.confirm('Uyarı', 'Application Pool u yeniden başlatmak istiyor musunuz?<br/><b>Bu işlem sizin oturumunuzu da sonlandıracaktır.!</b>', function (btn) {
				if (btn == "yes") {
					Ext.net.DirectMethods.IISRESET();
				}
			});
		}
	</script>
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Diller" Layout="FitLayout"
				Icon="FlagChecked" Closable="false" Minimizable="false" MinHeight="480" MinWidth="640">
				<Items>
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString" AutoExpandColumn="ulke"
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="ulke" Header="Ülke" MenuDisabled="true"
									Sortable="false"/>
								<ext:Column DataIndex="dil" Header="Dil" />
								<ext:Column DataIndex="kodlama" Header="Kodlama" />
								<ext:Column DataIndex="sira" Header="Sıra" Width="40" />
								<ext:Column DataIndex="ana_dil" Header="Ana Dil" Align="Center" Width="60" />
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:StringFilter DataIndex="dil" />
									<ext:StringFilter DataIndex="kodlama" />
									<ext:NumericFilter DataIndex="ulke_id" />
									<ext:NumericFilter DataIndex="sira" />
									...
									<ext:ListFilter DataIndex="ana_dil" Options="EVET,HAYIR" />
						...
						<TopBar>
							<ext:Toolbar ID="Toolbar1" runat="server">
								<Items>
								<ext:LinkButton ID="LabelUyari" runat="server" StyleSpec="float:left" Icon="Lightbulb" Text="Yaptığınız her hangi bir işlemin aktif olabilmesi için Application Pool 'un yeniden başlatılması gereklidir.">
								<Listeners>
								<Click Handler="IISRESET()" />
								</Listeners>
								</ext:LinkButton>
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				Hidden="true">
				<Items>
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Ekle_TextFieldDil" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Dil (Adı)" BlankText="Lütfen dil adını yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" EnableKeyEvents="true">
						<Listeners>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Ekle_TextFieldKodlama" MaxLength="10" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Kodlama" BlankText="Lütfen kodlamayı belirtin." Text="utf-8"
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldDil" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Dil (Adı)" BlankText="Lütfen dil adını yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" EnableKeyEvents="true">
						<Listeners>
							<KeyUp Handler="this.setValue(this.getValue().toUpperCase());" />
						</Listeners>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldKodlama" MaxLength="10" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Kodlama" BlankText="Lütfen kodlamayı belirtin." Text="utf-8"
						AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
