...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void StoreDoldurSabitler(ComboBox ComboBoxAdi, Store StoreAdi, int DilID)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			var SQL1 = (from p in SME.tbl_textler
			...
			var SQL2 = (from p in SME.tbl_sbt_sabit
	...
	[DirectMethod]
	public void EkleSabitlerDegistir(int DilID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.admin_id_gun,
					p.dil_id,
					sabit = SME.tbl_sbt_sabit.Where(k => k.id == p.sabitler_id).Select(x => x.aciklama).FirstOrDefault(),
	...
	protected void YeniKayitEkraniGetir(object sender, EventArgs e)
	{
		...
		StoreDoldurSabitler(Ekle_ComboBoxSabitler, StoreSabitler1, int.Parse(Ekle_ComboBoxDil.SelectedItem.Value));
		WindowEkle.Show();
	}
	...
	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			ta.sabitler_id = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxSabitler.Value.ToString().Trim()));
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_textler
						where p.id == ID
						select new
						{
							p.sabitler_id,
			...
			if (SQL2.Count() > 0)
			{
				...
				Duzenle_LabelSabit.Text = (SabitlerID != null ? SME.tbl_sbt_sabit.Where(x => x.id == SabitlerID).Select(x => x.aciklama).FirstOrDefault().ToString() : "-");
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_textler ta = (from p in SME.tbl_textler where p.id == PID select p).FirstOrDefault();
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_textler ta = (from p in SME.tbl_textler where p.id == ID select p).FirstOrDefault();
