...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void KullaniciAdiVarMesaji()
	{
		Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Error, MessageBox.Icon.ERROR, "İşlem başarısız", "Bu kullanıcı adı kullanılmaktadır. Lütfen başka bir kullanıcı adı belirtiniz.");
	}
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_adminler
							select new
							{
								p.id,
								p.kullanici_adi,
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			if (SQL.Count() > 0)
			{
				Duzenle_TextFieldAdiSoyadi.Text = SQL.Select(x => x.ad_soyad).FirstOrDefault();
				Duzenle_TextFieldKullaniciAdi.Text = SQL.Select(x => x.kullanici_adi).FirstOrDefault();
				Duzenle_EskiKullaniciAdi.Text = SQL.Select(x => x.kullanici_adi).FirstOrDefault();
				...
				if (Class.Fonksiyonlar.Admin.Kullanici.Kontroller.Root(SQL.Select(x => x.id).FirstOrDefault()))
				{
					#region Seçilen Kaydı Açan Root Değilse
					if (!Class.Fonksiyonlar.Admin.Kullanici.Kontroller.Root(Class.Fonksiyonlar.Admin.Genel.AdminID()))
					{
						Duzenle_TextFieldKullaniciAdi.Disabled = true;
						Duzenle_ComboBoxOnay.Disabled = true;
						Duzenle_Checbox.Disabled = true;
					}
					#endregion
				} 
				#endregion
