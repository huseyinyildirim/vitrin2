...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_firma_adresler
							select new
							{
								p.id,
								p.ulke_id,
	...
	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_firma_adresler ta = new tbl_firma_adresler();
			ta.firma_id = 1;
			...
			ta.adres = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextAreaAdres.Text.Trim());

			if (Ekle_TextFieldBaslik.Text != null)
			{
				ta.baslik = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldBaslik.Text.Trim()).ToUpper();
			}
			else
			{
				ta.baslik = null;
			...
			if (Ekle_TextFieldHaritaURL.Text != null)
			{
				if (!string.IsNullOrEmpty(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldHaritaURL.Text.Trim())))
				{
					ta.harita_url = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldHaritaURL.Text.Trim());
				}
				else
				{
					ta.harita_url = null;
			...
			ta.onay = OnayBool;
			SME.AddTotbl_firma_adresler(ta);
			SME.SaveChanges();
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_firma_adresler
						where p.id == ID
						select new
						{
							p.id,
			...
			if (SQL2.Count() > 0)
			{
				Duzenle_EskiBayrak.Text = SQL2.Select(x => x.ikon).FirstOrDefault();
				Duzenle_TextFieldBaslik.Text = SQL2.Select(x => x.baslik).FirstOrDefault();
				Duzenle_TextFieldHaritaURL.Text = SQL2.Select(x => x.harita_url).FirstOrDefault();
				Duzenle_ComboBoxUlke.SelectedItem.Value = SQL2.Select(x => x.ulke_id).FirstOrDefault().ToString();
				Duzenle_TextAreaAdres.Text = SQL2.Select(x => x.adres).FirstOrDefault();
				...
				SB.Append(SQL2.Select(x => x.ikon.Replace(".png", "").Replace(".jpg", "").Replace(".gif", "").Replace("_", " ").ToUpper()).FirstOrDefault());
				SB.Append("</span> <img style=\"vertical-align:middle\" alt=\"");
				SB.Append(SQL2.Select(x => x.ikon.Replace(".png", "").Replace(".jpg", "").Replace(".gif", "").Replace("_", " ").ToUpper()).FirstOrDefault());
				SB.Append("\" src=\"../images/flag/");
				SB.Append(SQL2.Select(x => x.ikon).FirstOrDefault());
				SB.Append("\"></div>");
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_firma_adresler ta = (from p in SME.tbl_firma_adresler where p.id == PID select p).FirstOrDefault();

			if (Duzenle_TextFieldBaslik.Text != null)
			{
				ta.baslik = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldBaslik.Text.Trim()).ToUpper();
			}
			else
			{
				ta.baslik = null;
			...
			ta.adres = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextAreaAdres.Text.Trim());
			...
			{
				if (!string.IsNullOrEmpty(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldHaritaURL.Text.Trim())))
				{
					ta.harita_url = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldHaritaURL.Text.Trim());
				}
				else
				{
					ta.harita_url = null;
	...
	protected void SiralamaGuncelle(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			if (YeniSira != 0)
			{
				...
				if (SQL.Count() > 0)
				{
					tbl_firma_adresler td1 = (from p in SME.tbl_firma_adresler where p.sira == YeniSira select p).FirstOrDefault();
					td1.sira = 0;
					...
					Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Information, MessageBox.Icon.INFO, "Aynı Kayıt", "Vermek istediğiniz sıra numarası başka bir kayda (" + td1.id + ") aitti ve eski kaydın sırası da 0 olarak değiştirildi.");
				}

				tbl_firma_adresler td2 = (from p in SME.tbl_firma_adresler where p.id == PID select p).FirstOrDefault();
				td2.sira = YeniSira;
			...
			{
				tbl_firma_adresler td3 = (from p in SME.tbl_firma_adresler where p.id == PID select p).FirstOrDefault();
				td3.sira = 0;
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_firma_adresler ta = (from p in SME.tbl_firma_adresler where p.id == ID select p).FirstOrDefault();
