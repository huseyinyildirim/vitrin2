...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void StoreDoldurURL(Store StoreAdi)
	{
		...
		#region Listeden SQL
		var URL = (from p in L
				   where (p != "tag" && p != "categori" && p != Class.Fonksiyonlar.Genel.OzelAyar("URL") + "-product" && p != "brand" && p != "search")
				   select new
				   {
					   url = p
				   }).AsEnumerable();
	...
	protected void SiralamaGuncelle(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			if (YeniSira != 0)
			{
				...
				if (SQL.Count() > 0)
				{
					tbl_menuler td1 = (from p in SME.tbl_menuler where p.sira == YeniSira && p.dil_id == DilID select p).FirstOrDefault();
					td1.sira = 0;
				...
				tbl_menuler td2 = (from p in SME.tbl_menuler where p.id == PID && p.dil_id == DilID select p).FirstOrDefault();
				td2.sira = YeniSira;
			...
			{
				tbl_menuler td3 = (from p in SME.tbl_menuler where p.id == PID && p.dil_id == DilID select p).FirstOrDefault();
				td3.sira = 0;
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_menuler ta = (from p in SME.tbl_menuler where p.id == ID select p).FirstOrDefault();
