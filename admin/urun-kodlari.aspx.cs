...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected bool KodKontrol(string Kod)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_kodlar
					   where p.kod == Kod
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_kodlar
						where p.id == ID
						select new
						{
							p.id,
			...
			if (SQL2.Count() > 0)
			{
				Duzenle_TextFieldKod.Text = SQL2.Select(x => x.kod).FirstOrDefault();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_kodlar ta = (from p in SME.tbl_kodlar where p.id == PID select p).FirstOrDefault();
			ta.kod = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldKod.Text.Trim()).ToUpper();
