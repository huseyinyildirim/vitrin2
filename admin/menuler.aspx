<%@ Page Language="C#" AutoEventWireup="true" CodeFile="menuler.aspx.cs" Inherits="yonetim_Kullanici"
...
<body>
	<script type="text/javascript">
		...
		var DuzenleKontrol = function () {
			...
			else if (!Duzenle_TextFieldMenu.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Duzenle_ComboBoxURL.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir URL seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
		...
		var KaydetKontrol = function () {
			...
			else if (!Ekle_TextFieldMenu.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Ekle_ComboBoxURL.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir URL seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_menuler&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Menüler" Layout="FitLayout"
					...
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString"
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="ad" Header="Menü" />
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:StringFilter DataIndex="menu" />
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Ekle_TextFieldMenu" MaxLength="25" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Menü (Adı)" BlankText="Lütfen menü adını yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
					<ext:ComboBox ID="Ekle_ComboBoxURL" FieldLabel="URL" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" runat="server"
						DisplayField="url" ValueField="url" Editable="true" TypeAhead="true" Mode="Local"
						ForceSelection="true" SelectOnFocus="true" EmptyText="URL Seçiniz...">
						<Store>
							<ext:Store ID="StoreUrl1" runat="server" WarningOnDirty="false">
								<Reader>
									<ext:JsonReader>
										<Fields>
											<ext:RecordField Name="url" />
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldMenu" MaxLength="25" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Menü (Adı)" BlankText="Lütfen menü adını yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
					<ext:ComboBox ID="Duzenle_ComboBoxURL" FieldLabel="URL" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" runat="server"
						DisplayField="url" ValueField="url" Editable="true" TypeAhead="true" Mode="Local"
						ForceSelection="true" SelectOnFocus="true" EmptyText="URL Seçiniz...">
						<Store>
							<ext:Store ID="StoreUrl2" runat="server" WarningOnDirty="false">
								<Reader>
									<ext:JsonReader>
										<Fields>
											<ext:RecordField Name="url" />
