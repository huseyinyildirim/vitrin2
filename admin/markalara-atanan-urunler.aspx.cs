...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_urunler_markalar
							select new
							{
								p.id,
								p.marka_id,
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.admin_id_ek,
					p.admin_id_gun,
					marka_ziyaret = SME.tbl_ziyaretler.Where(k => k.marka_id == p.marka_id).Count(),
					...
					marka = SME.tbl_markalar.Where(k => k.id == p.marka_id).Select(x => x.ad).FirstOrDefault(),
	...
	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			tbl_urunler_markalar ta = new tbl_urunler_markalar();
			ta.marka_id = int.Parse(Ekle_ComboBoxMarka.SelectedItem.Value.ToString());
			ta.urun_id = int.Parse(Ekle_ComboBoxUrun.SelectedItem.Value.ToString());
			ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
			SME.AddTotbl_urunler_markalar(ta);
			SME.SaveChanges();
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_urunler_markalar
						where p.id == ID
						select new
						{
							p.urun_id,
			...
			if (SQL2.Count() > 0)
			{
				...
				Duzenle_ComboBoxMarka.SelectedItem.Value = SQL2.Select(x => x.marka_id).FirstOrDefault().ToString();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_urunler_markalar ta = (from p in SME.tbl_urunler_markalar where p.id == PID select p).FirstOrDefault();
			ta.marka_id = int.Parse(Duzenle_ComboBoxMarka.SelectedItem.Value.ToString());
