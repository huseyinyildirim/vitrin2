...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	public void NodeSurukle(string NodeID, string YeniUstKategori)
	{
		try
		{
			...
			{
				tbl_dosyalar_kategori tbk = (from p in SME.tbl_dosyalar_kategori where p.id == INodeID select p).FirstOrDefault();
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_dosyalar_kategori
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					ust_kategori = SME.tbl_dosyalar_kategori.Where(k => k.id == p.kategori_id).Select(x => x.ad).FirstOrDefault()
			...
			if (TreePanel1.CheckedNodes != null)
			{
				foreach (var i in TreePanel1.CheckedNodes)
				{
					...
					if (Ekle_TextAreaAciklama.Text!=null)
					{
						...
						{
							ta.aciklama = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextAreaAciklama.Text.Trim());
						}
						else
						{
							ta.aciklama = null;
					...
					SME.AddTotbl_dosyalar_kategori(ta);
					SME.SaveChanges();
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_dosyalar_kategori
						where p.id == ID
						select new
						{
							p.ad,
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_dosyalar_kategori ta = (from p in SME.tbl_dosyalar_kategori where p.id == PID select p).FirstOrDefault();
			...
			if (Duzenle_TextAreaAciklama.Text != null)
			{
				...
				{
					ta.aciklama = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextAreaAciklama.Text.Trim());
				}
				else
				{
					ta.aciklama = null;
