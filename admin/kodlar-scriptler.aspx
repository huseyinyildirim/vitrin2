<%@ Page Language="C#" AutoEventWireup="true" CodeFile="kodlar-scriptler.aspx.cs"
...
<body>
	<script type="text/javascript">
		...
		var DuzenleKontrol = function () {
			if (Duzenle_ComboBoxMenu.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir menü seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Duzenle_ComboBoxMenu.getSelectedIndex() == -1) {
			...
			else if (!Duzenle_TextAreaScript.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
		...
		var KaydetKontrol = function () {
			if (Ekle_ComboBoxMenu.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir menü seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			else if (Ekle_ComboBoxMenu.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir resim seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
			...
			else if (!Ekle_TextAreaScript.validate()) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen zorunlu tüm alanları doldurun.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
		}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_scriptler&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Kodlar (Scriptler)"
				Layout="FitLayout" Icon="ScriptCodeRed" Closable="false" Minimizable="false" MinHeight="480"
				MinWidth="640">
				<Items>
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString"
						AutoExpandColumn="baslik" OnBeforeClientInit="GridPanelYuklenmedenOnce" AnchorHorizontal="100%"
								...
								<Reader>
									<ext:JsonReader IDProperty="id">
										<Fields>
											...
											<ext:RecordField Name="menu_id" Type="Int" />
											<ext:RecordField Name="menu" />
											<ext:RecordField Name="menu_url" />
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="menu_id" Header="Menü ID" Hidden="true" />
								<ext:Column DataIndex="menu" Header="Menü (Sayfa)" MenuDisabled="true" Sortable="false" />
								<ext:Column DataIndex="menu_url" Header="Menü (Sayfa) URL" MenuDisabled="true" Sortable="false">
								<Renderer Fn="URLGoster" />
								</ext:Column>
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:NumericFilter DataIndex="menu_id" />
						...
						<Buttons>
							...
							<ext:Button runat="server" ID="Button2" Text="Seçilenleri Onayla" Icon="Tick">
								<DirectEvents>
							...
							<ext:Button runat="server" ID="Button3" Text="Seçilenlerden Onayı Kaldır" Icon="Cross">
								<DirectEvents>
							...
							<ext:Button runat="server" ID="Button4" Text="Seçilenleri Sil" Icon="Delete">
								<DirectEvents>
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				Hidden="true">
				<Items>
					<ext:ComboBox ID="Ekle_ComboBoxMenu" FieldLabel="Menü" AnchorHorizontal="96%" runat="server"
						DisplayField="ad" ValueField="id" Editable="true" TypeAhead="true" Mode="Local"
						ForceSelection="true" SelectOnFocus="true" EmptyText="Menü Seçiniz..." IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan">
						<Store>
							<ext:Store ID="StoreMenu1" runat="server" WarningOnDirty="false">
								<Reader>
									<ext:JsonReader>
										<Fields>
											<ext:RecordField Name="id" Type="Int" />
					...
					<ext:TextArea ID="Ekle_TextAreaScript" MsgTarget="Side" AllowBlank="false" BlankText="Lütfen içeriği yazın."
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:TextField ID="Duzenle_MenuID" Hidden="true" runat="server" />
									<ext:ComboBox ID="Duzenle_ComboBoxMenu" FieldLabel="Menü" AnchorHorizontal="96%" runat="server"
						DisplayField="ad" ValueField="id" Editable="true" TypeAhead="true" Mode="Local"
						ForceSelection="true" SelectOnFocus="true" EmptyText="Menü Seçiniz..." IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan">
						<Store>
							<ext:Store ID="StoreMenu2" runat="server" WarningOnDirty="false">
								<Reader>
									<ext:JsonReader>
										<Fields>
											<ext:RecordField Name="id" Type="Int" />
					...
					<ext:TextField MsgTarget="Side" AllowBlank="false" ID="Duzenle_TextFieldBaslik" MaxLength="255" MaxLengthText="Maksimum karakter sınırını aştınız."
						runat="server" FieldLabel="Başlık" BlankText="Lütfen bir başlık yazın." AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
					<ext:TextArea ID="Duzenle_TextAreaScript" MsgTarget="Side" AllowBlank="false" BlankText="Lütfen içeriği yazın."
						runat="server" FieldLabel="İçerik" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" />
				...
				<Buttons>
					<ext:Button ID="Duzenle_Kaydet" runat="server" Text="Güncelle" Icon="Accept" Type="Submit">
						<Listeners>
