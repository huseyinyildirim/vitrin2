...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void StoreDoldurMenu(Store StoreAdi)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int DilID = (from p in SME.tbl_diller
	...
	protected bool ScriptKontrol(int MenuID)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_scriptler
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				#region İlk (Ham) SQL
				var SQL1 = (from p in SME.tbl_scriptler
							select new
							{
								p.id,
								p.menu_id,
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.admin_id_gun,
					p.menu_id,
					menu = SME.tbl_menuler.Where(k => k.id == p.menu_id).Select(x => x.ad).FirstOrDefault(),
					menu_url = SME.tbl_menuler.Where(k => k.id == p.menu_id).Select(x => x.url).FirstOrDefault(),
	...
	{
		StoreDoldurMenu(StoreMenu1);
		WindowEkle.Show();
	}

	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		if (!ScriptKontrol(int.Parse(Ekle_ComboBoxMenu.SelectedItem.Value.ToString())))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				tbl_scriptler ta = new tbl_scriptler();
				ta.menu_id = int.Parse(Ekle_ComboBoxMenu.SelectedItem.Value.ToString());
				ta.baslik = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldBaslik.Text.Trim());
				ta.script = Ekle_TextAreaScript.Text.Trim().Replace(@"'", "\'");
				ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
				ta.onay = OnayBool;
				SME.AddTotbl_scriptler(ta);
				SME.SaveChanges();
		...
		{
			HataMesaji("Aynı sayfaya daha önce bir script eklenmiş.<br/>Lütfen farklı bir sayfa belirtin!");
		}
	} 
	...
	{
		StoreDoldurMenu(StoreMenu2);
		Duzenle_ID.Text = ID.ToString();

		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_scriptler
						where p.id == ID
						select new
						{
							p.menu_id,
			...
			if (SQL2.Count() > 0)
			{
				Duzenle_MenuID.Text = SQL2.Select(x => x.menu_id).FirstOrDefault().ToString();
				Duzenle_ComboBoxMenu.SelectedItem.Value = SQL2.Select(x => x.menu_id).FirstOrDefault().ToString();
				Duzenle_TextFieldBaslik.Text = SQL2.Select(x => x.baslik).FirstOrDefault();
				Duzenle_TextAreaScript.Html = SQL2.Select(x => x.script).FirstOrDefault();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_scriptler ta = (from p in SME.tbl_scriptler where p.id == PID select p).FirstOrDefault();
			ta.baslik = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldBaslik.Text.Trim());
			ta.script = Duzenle_TextAreaScript.Text.Trim().Replace(@"'", "\'");
			ta.onay = OnayBool;
			ta.admin_id_gun = Class.Fonksiyonlar.Admin.Genel.AdminID();

			if (Duzenle_MenuID.Text != Duzenle_ComboBoxMenu.SelectedItem.Value.ToString())
			{
				if (!ScriptKontrol(int.Parse(Duzenle_ComboBoxMenu.SelectedItem.Value.ToString())))
				{
					ta.menu_id = int.Parse(Duzenle_ComboBoxMenu.SelectedItem.Value.ToString());
					SME.SaveChanges();
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_scriptler ta = (from p in SME.tbl_scriptler where p.id == ID select p).FirstOrDefault();
