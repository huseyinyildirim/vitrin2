...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	{
		return Class.Fonksiyonlar.Genel.ResimleriListele(Class.Degiskenler.Site.Yollar.Header)
			.Select(s => new
			{
				resim = s.Replace(Class.Degiskenler.Site.Yollar.Header, ""),
			}).AsEnumerable();
	}
	...
	protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				var SQL1 = (from p in SME.tbl_headerlar
	...
	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_headerlar ta = new tbl_headerlar();
			...
			ta.resim = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxResim.Value.ToString().Trim());
			ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
			ta.onay = OnayBool;
			SME.AddTotbl_headerlar(ta);
			SME.SaveChanges();
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_headerlar
						where p.id == ID
						select new
						{
							p.dil_id,
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			...
			tbl_headerlar ta = (from p in SME.tbl_headerlar where p.id == PID select p).FirstOrDefault();
			...
			ta.resim = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxResim.Value.ToString().Trim());
			ta.onay = OnayBool;
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_headerlar ta = (from p in SME.tbl_headerlar where p.id == ID select p).FirstOrDefault();
