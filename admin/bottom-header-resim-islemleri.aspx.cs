...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		else
		{
			...
			Class.Fonksiyonlar.JavaScript.Ekle("var OtherYolu = \"" + Class.Degiskenler.Site.Yollar.Diger.Replace(@"\", "\\\\") + "\";");
			#endregion
	...
	{
		return Class.Fonksiyonlar.Genel.ResimleriListele(Class.Degiskenler.Site.Yollar.Diger)
			.Select(s => new
			{
				resim = s.Replace(Class.Degiskenler.Site.Yollar.Diger, ""),
			}).AsEnumerable();
	}

	protected void StoreDoldurResim(Store StoreAdi)
	{
		StoreAdi.DataSource = ResimleriGetir();
		StoreAdi.DataBind();
	}
	...
	protected bool KayitKontrol(int DilID,int Yer,int Konum)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_ikonlar
					   where p.dil_id == DilID && p.yer == Yer && p.konum == Konum
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			try
			{
				...
				var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
				{
					...
					p.alt,
					p.yer,
					yers = (p.yer == 0 ? "ÜST" : "ALT"),
					p.konum,
					konums = (p.konum == 0 ? "SOL" : "SAĞ"),
					p.tarih_ek,
					p.tarih_gun,
					p.admin_id_ek,
					p.admin_id_gun,
	...
	{
		if (!KayitKontrol(int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxDil.Value.ToString().Trim())), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxYer.Value.ToString().Trim())), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxKonum.Value.ToString().Trim()))))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				tbl_ikonlar ta = new tbl_ikonlar();
				ta.dil_id = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxDil.Value.ToString().Trim()));
				ta.alt = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_TextFieldBaslik.Text.ToString().Trim());
				ta.ikon = Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxResim.Value.ToString().Trim());
				ta.yer = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxYer.Value.ToString().Trim()));
				ta.konum = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Ekle_ComboBoxKonum.Value.ToString().Trim()));
				ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
				ta.onay = OnayBool;
				SME.AddTotbl_ikonlar(ta);
				SME.SaveChanges();
		...
		{
			HataMesaji("Aynı dile ait kayıt mevcut.<br/>Lütfen farklı bir dil belirtin!");
		}
	}
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_ikonlar
						where p.id == ID
						select new
						{
							p.dil_id,
			...
			if (SQL2.Count() > 0)
			{
				...
				Duzenle_ComboBoxResim.SelectedItem.Value = SQL2.Select(x => x.ikon).FirstOrDefault();
				Duzenle_TextFieldBaslik.Text = SQL2.Select(x => x.alt).FirstOrDefault();
				Duzenle_ComboBoxKonum.SelectedItem.Value = SQL2.Select(x => x.konum).FirstOrDefault().ToString();
				Duzenle_ComboBoxYer.SelectedItem.Value = SQL2.Select(x => x.yer).FirstOrDefault().ToString();
	...
	{
		if (!KayitKontrol(int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxDil.Value.ToString().Trim())), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxYer.Value.ToString().Trim())), int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxKonum.Value.ToString().Trim()))))
		{
			#region Onay Boolean Çevirme
			bool OnayBool = false;
			...
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				tbl_ikonlar ta = (from p in SME.tbl_ikonlar where p.id == PID select p).FirstOrDefault();
				ta.dil_id = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxDil.Value.ToString().Trim()));
				ta.alt = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_TextFieldBaslik.Text.ToString().Trim());
				ta.ikon = Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxResim.Value.ToString().Trim());
				ta.yer = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxYer.Value.ToString().Trim()));
				ta.konum = int.Parse(Class.Fonksiyonlar.Genel.SQLTemizle(Duzenle_ComboBoxKonum.Value.ToString().Trim()));
				ta.onay = OnayBool;
	...
	public void OnayKaldir(int ID)
	{
		...
		else
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				tbl_ikonlar ta = (from p in SME.tbl_ikonlar where p.id == ID select p).FirstOrDefault();
