...
public partial class yonetim_Kullanici : System.Web.UI.Page
{
	...
	protected void StoreDoldurEtiket(Store StoreAdi)
	{
	...
	protected bool EtiketKontrol(int EtiketID, int UrunID)
	{
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			int EID = (from p in SME.tbl_urunler_etiketler
					   where p.etiket_id == EtiketID && p.urun_id == UrunID
	...
	{
		StoreDoldurEtiket(StoreEtiket1);
	...
	protected void YeniKayitYap(object sender, DirectEventArgs e)
	{
		if (!EtiketKontrol(int.Parse(Ekle_ComboBoxEtiket.SelectedItem.Value.ToString()),int.Parse(Ekle_ComboBoxUrun.SelectedItem.Value.ToString())))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				SME.AddTotbl_urunler_etiketler(ta);
				SME.SaveChanges();
		...
		{
			HataMesaji("Aynı etiket daha önce bur ürüne atanmış.<br/>Lütfen farklı bir etiket veya farklı bir ürün belirtin!");
		}
	} 
	...
	public void GuncellemeEkraniGetir(int ID)
	{
		...
		using (BaglantiCumlesi SME = new BaglantiCumlesi())
		{
			var SQL2 = (from p in SME.tbl_urunler_etiketler
						where p.id == ID
						select new
						{
							p.urun_id,
			...
			if (SQL2.Count() > 0)
			{
				StoreDoldurEtiket(StoreEtiket2);
				StoreDoldurUrun(StoreUrun2);

				Duzenle_ComboBoxEtiket.SelectedItem.Value = SQL2.Select(x => x.etiket_id).FirstOrDefault().ToString();
	...
	protected void GuncellemeYap(object sender, DirectEventArgs e)
	{
		if (!EtiketKontrol(int.Parse(Duzenle_ComboBoxEtiket.SelectedItem.Value.ToString()), int.Parse(Duzenle_ComboBoxUrun.SelectedItem.Value.ToString())))
		{
			using (BaglantiCumlesi SME = new BaglantiCumlesi())
			{
				...
				tbl_urunler_etiketler ta = (from p in SME.tbl_urunler_etiketler where p.id == PID select p).FirstOrDefault();
