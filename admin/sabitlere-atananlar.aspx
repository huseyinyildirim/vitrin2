<%@ Page Language="C#" AutoEventWireup="true" CodeFile="sabitlere-atananlar.aspx.cs"
...
<body>
	<script type="text/javascript">
		...
		var EkleSabitlerDegistir = function (t) {
			Ext.net.DirectMethods.EkleSabitlerDegistir(t);
		}
		...
		var WindowDuzenleClear = function () {
			for (var i = 0; i <= WindowDuzenle.items.items.length; i++) {
				if (WindowEkle.items.items[i].xtype == "textarea") {
		...
		var DuzenleKontrol = function () {
			if (!Duzenle_TextAreaText.validate()) {
		...
		var KaydetKontrol = function () {
			...
			else if (Ekle_ComboBoxSabitler.getSelectedIndex() == -1) {
				Ext.Msg.show({ title: 'Hata', msg: 'Lütfen bir sabit seçin.', buttons: Ext.Msg.OK, icon: Ext.Msg.ERROR });
				return false;
			}
				...
				case "IslemKaydi":
					...
					var win = new Ext.Window({
						...
						autoLoad: {
							...
							showMask: true,
							mode: "iframe",
							url: "isleme-ait-kayitlar.aspx?tablo_adi=tbl_textler&kayit_id=" + record.data.id,
							scripts: true
						},
						buttonAlign: 'right',
						buttons: [
	...
	<ext:Viewport ID="Viewport1" runat="server" Layout="FitLayout" StyleSpec="background-color:#F0F0F0">
		<Items>
			<ext:Window ID="Window1" runat="server" Maximized="true" Title="Sabitlere Atananlar"
				Layout="FitLayout" Icon="CalendarLink" Closable="false" Minimizable="false" MinHeight="480"
				MinWidth="640">
				<Items>
					<ext:GridPanel ID="GridPanel1" runat="server" ColumnLines="true" SortType="AsUCString"
						AutoExpandColumn="sabit" OnBeforeClientInit="GridPanelYuklenmedenOnce" AnchorHorizontal="100%"
								...
								<Reader>
									<ext:JsonReader IDProperty="id">
										<Fields>
											...
											<ext:RecordField Name="sabitler_id" Type="Int" />
											<ext:RecordField Name="sabit" />
						...
						<ColumnModel runat="server">
							<Columns>
								...
								<ext:Column DataIndex="sabitler_id" Header="Sabitler ID" Hidden="true" />
								<ext:Column DataIndex="sabit" Header="Sabit" MenuDisabled="true" Sortable="false" />
								<ext:Column DataIndex="text" Header="Text" />
						...
						<Plugins>
							<ext:GridFilters runat="server" ID="GridFilters1" Local="true">
								<Filters>
									...
									<ext:NumericFilter DataIndex="sabitler_id" />
			...
			<ext:Window ID="WindowEkle" runat="server" Modal="false" Resizable="True" Height="500"
				...
				<Items>
					<ext:ComboBox ID="Ekle_ComboBoxDil" FieldLabel="Dil" AnchorHorizontal="100%" runat="server"
						...
						<Listeners>
							<Select Handler="EkleSabitlerDegistir(Ekle_ComboBoxDil.getValue())" />
						</Listeners>
					</ext:ComboBox>
					<ext:ComboBox ID="Ekle_ComboBoxSabitler" FieldLabel="Sabit" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan"
						runat="server" DisplayField="aciklama" ValueField="id" Editable="true" TypeAhead="true"
						Mode="Local" ForceSelection="true" SelectOnFocus="true" EmptyText="Sabit Seçiniz...">
						<Store>
							<ext:Store ID="StoreSabitler1" runat="server" WarningOnDirty="false">
								<Reader>
									<ext:JsonReader>
										<Fields>
											<ext:RecordField Name="id" Type="Int" />
					...
					<ext:TextArea ID="Ekle_TextAreaText" MsgTarget="Side" AllowBlank="false" BlankText="Lütfen içeriği yazın."
						runat="server" FieldLabel="İçerik" AnchorHorizontal="96%" IndicatorIcon="BulletStar" IndicatorTip="Zorunlu alan" Height="250" />
			...
			<ext:Window ID="WindowDuzenle" runat="server" Modal="true" Resizable="True" Height="500"
				...
				<Items>
					...
					<ext:Label ID="Duzenle_LabelSabit" FieldLabel="Sabit" runat="server" />
