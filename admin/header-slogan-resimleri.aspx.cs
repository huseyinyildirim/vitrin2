...
public partial class yonetim_dosyalar2 : System.Web.UI.Page
{
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		else
		{
			...
			{
				VerileriGetir();
				Label2.Html = "İlgili resimler " + "<b><u>" + Class.Degiskenler.Site.Yollar.Header + "</u></b>" + " klasöründen gösterilmektedir.";
			}
			#endregion
		}
		#endregion
	...
	protected IEnumerable ResimleriGetir()
	{
			...
			.Select(s => new
		{
		...
		}).Select((p, k) => new
			{
				...
				url = "ashx/image.ashx?n=H&i=" + p.resim + "&w=80&h=80&k=t",
				p.yol,
				p.resim,
			}).AsEnumerable();
	}
	...
	protected void DosyaGonder()
	{
		...
		if (boyut > 1)
		{
			if (uzanti == "jpg" || uzanti == "gif" || uzanti == "bmp" || uzanti == "png")
			{
				if (boyut < 2201600)
				{
					if (!File.Exists(Class.Degiskenler.Site.Yollar.Header + GelenDosya.FileName))
					{
						try
						{
							byte[] B = new byte[boyut];
							GelenDosya.InputStream.Read(B, 0, boyut);
							Class.Fonksiyonlar.Genel.DosyaKaydet(B, Class.Degiskenler.Site.Yollar.Header + GelenDosya.FileName);
